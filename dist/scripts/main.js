// scripts/main.ts
import { EntityComponentTypes as EntityComponentTypes5, system as system2, world as world7 } from "@minecraft/server";

// scripts/constants/Compagnons.ts
var COMPAGNONS = /* @__PURE__ */ new Map();
...
// scripts/constants/Commands.ts
var COMMANDS = [];

// scripts/functions/initCommand.ts
function initCommands(props) {
  const { name } = props;
  if (!name) throw new Error("name is required on command initialisation");
  COMMANDS.push(props);
}

// scripts/commands/help.ts
initCommands({
  name: "help",
  desc: "Help command",
  alias: ["h"],
  function: (args, player) => {
    if (args?.length > 0) {
      ...
      if (commandInfo) {
        const aliases = commandInfo.alias?.length ? commandInfo.alias.join(", ") : "none";
        return player.sendMessage(
          `\xA77Command \xA7e${commandInfo.name}\xA77 :
\xA7aDescription \xA7r: ${commandInfo.desc}
\xA7bAliases \xA7r: ${aliases}
`
        );
      } else {
        return player.sendMessage("\xA7cUnkown command");
      }
    } else {
      return player.sendMessage(
        `\xA77Commandes list :
` + COMMANDS.map((c) => `\xA7e${c.name}\xA77`).join(", ")
      );
    }
  }
});

// scripts/events/chat.ts
import { world } from "@minecraft/server";

// scripts/constants/isDebug.ts
var isDebug = true;

// scripts/functions/debugLog.ts
...
// scripts/events/entity.ts
import { EntityComponentTypes as EntityComponentTypes2, world as world3 } from "@minecraft/server";

// scripts/class/CompagnonDBManager.ts
import { EntityComponentTypes, world as world2 } from "@minecraft/server";
var CompagnonDBManager = class _CompagnonDBManager {
  static updateCompagnonData(player, property) {
    const currentProperty = _CompagnonDBManager.getCompagnon(player) ?? {};
  ...
  static hasCompagnon(player) {
    return world2.getDynamicProperty(`compagnon_${player.id}`) !== void 0;
  }
  static getCompagnon(player) {
    return _CompagnonDBManager.hasCompagnon(player) ? JSON.parse(world2.getDynamicProperty(`compagnon_${player.id}`)) : void 0;
  }
  static removeCompagnonData(player) {
    world2.setDynamicProperty(`compagnon_${player.id}`, void 0);
  }
  static createCompagnonData(player, compagnon) {
    _CompagnonDBManager.updateCompagnonData(player, {
      name: compagnon.name,
  ...
  static getAllForExistingPlayer() {
    ...
    for (const player of players) {
      if (_CompagnonDBManager.hasCompagnon(player)) {
        const data = _CompagnonDBManager.getCompagnon(player);
        if (data) {
...
// scripts/constants/compagnonType.ts
var COMPAGNON_TYPE = "faouzdev:compagnon";
...
world3.afterEvents.entityHitEntity.subscribe((event) => {
  ...
  if (damagingEntity.getComponent(EntityComponentTypes2.TypeFamily)?.hasTypeFamily("player")) {
...
world3.afterEvents.itemUse;

// scripts/events/player.ts
import { EntityComponentTypes as EntityComponentTypes4, world as world5 } from "@minecraft/server";

// scripts/class/CompagnonManager.ts
import {
  world as world4,
...
import { getPlayerSkin, LookDuration } from "@minecraft/server-gametest";
...
// scripts/functions/checkforBestItem.ts
import { EquipmentSlot } from "@minecraft/server";
var EQUIPMENT_SCORES = {
  [EquipmentSlot.Head]: {
...
function checkforBestItem(currentItem, inventory, slot, purpose = "combat") {
...
// scripts/constants/foodMobs.ts
var FOOD_MOBS = /* @__PURE__ */ new Set([
  "minecraft:cow",
  // raw beef
  "minecraft:pig",
  // raw porkchop
...
// scripts/functions/getRandomPointAround.ts
function getRandomPointAround(center, minDistance = 3, maxDistance = 10) {
...
// scripts/functions/createCube.ts
import { BlockVolume as BlockVolume2 } from "@minecraft/server";
function createCube(center, distance) {
...
// scripts/functions/isBedOccuped.ts
function isBedOccupied(block) {
  const isHead = block.permutation.getState("head_piece_bit");
  if (isHead === true) {
    debugLog("[isBedOccupied] we found head");
    return block.permutation.getState("occupied_bit") === true;
  }
  ...
  if (head.permutation.getState("head_piece_bit") !== true) {
    debugLog("[isBedOccupied] head not found with direction search");
    return false;
  }
  debugLog("[isBedOccupied] head found");
  return head.permutation.getState("occupied_bit") === true;
}

// scripts/functions/checkForBestFood.ts
var FOOD_SCORES = {
  // --- Excellents (Score 60+) ---
  "minecraft:enchanted_golden_apple": 100,
  // Optionnel (Golden)
  "minecraft:golden_apple": 90,
  // Optionnel (Golden)
  "minecraft:golden_carrot": 85,
  "minecraft:cooked_beef": 80,
  // Steak
...
function checkForBestFood(inventory, includeGolden = false, includeNegative = false) {
  let bestItem = void 0;
  let bestScore = -1;
  let slotIndex = void 0;
  for (let i = 0; i < inventory.size; i++) {
    const item = inventory.getItem(i);
    if (!item) continue;
...
// scripts/functions/safestDirectionFromMob.ts
function safestDirectionFromMob(mobPosition, companionPosition, dimension) {
  const directions = [
    { x: 1, y: 0, z: 0 },
    { x: -1, y: 0, z: 0 },
    { x: 0, y: 0, z: 1 },
...
function evaluateDirection(direction, companionPosition, mobPosition, dimension) {
  const distance = 5;
  let score = 0;
...
// scripts/class/CompagnonManager.ts
var CompagnonManager = class {
  //=================================================
  // #endregion Variable Declaration
  //==================================================
  constructor(_compagnon, _owner) {
    this._compagnon = _compagnon;
    this._owner = _owner;
    //=================================================
    ...
    this.farming_behavior_datas = { checkpoint: null, action: null };
    this.sleep_behavior_data = {
      noBedFoundMessageCooldown: 0
    };
    this.eat_behavior_data = { startEating: false };
    this.crop_farming_behavior_data = {
      area: void 0,
      chest: {
        chestPosition: void 0,
        chestContainerComponents: void 0,
  ...
  getInventoryComponent() {
    return this._compagnon.getComponent(EntityComponentTypes3.Inventory);
  }
  getEquipableComponent() {
    return this._compagnon.getComponent(EntityComponentTypes3.Equippable);
  }
  tellOnwer(message) {
    this._owner.sendMessage(`\xA78[\xA7b${this._compagnon.name}\xA78] \xA77\u2192 \xA7r${message}`);
  }
  /**
  ...
  armorUpdater() {
    ...
    for (const armorSlot of [EquipmentSlot2.Head, EquipmentSlot2.Chest, EquipmentSlot2.Legs, EquipmentSlot2.Feet]) {
  ...
  offHandUpdater() {
    const equipableComponent = this.getEquipableComponent();
    const inventoryComponent = this.getInventoryComponent();
    const hasOffhandTool = equipableComponent.getEquipment(EquipmentSlot2.Offhand);
    const bestOffhandTool = checkforBestItem(hasOffhandTool, inventoryComponent.container, EquipmentSlot2.Offhand);
    if (bestOffhandTool.shouldChange && bestOffhandTool.index !== void 0) {
      debugLog(`[OffHandUpdater] - Compagnon should change offhand tool : ${bestOffhandTool.bestItem?.type}`);
      try {
        equipableComponent.setEquipment(EquipmentSlot2.Offhand, bestOffhandTool.bestItem?.clone());
        inventoryComponent.container.setItem(bestOffhandTool.index);
        debugLog("[OffHandUpdater] - Compagnon offhand tool set on slot " + EquipmentSlot2.Offhand);
      } catch (error) {
  ...
  UtilsItemsOrganiser() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (bestItem.shouldChange && bestItem.index !== void 0) {
  ...
  move(target) {
    const targetData = target && typeof target === "object" && "type" in target ? target : target && typeof target === "object" && "location" in target ? { type: "entity", entity: target } : { type: "location", location: target };
    const targetEntity = targetData.type === "entity" ? targetData.entity : null;
    const targetLocation = targetData.type === "location" ? targetData.location : targetEntity ? targetEntity.location : null;
    if (!targetLocation) {
  ...
  updateNameTag() {
    const health = this._compagnon.getComponent(EntityComponentTypes3.Health).currentValue;
    const hearts = Math.round(health / 2);
    this._compagnon.nameTag = `\xA7l\xA7f${this.compagnon.name}\xA7r
\xA7c\u2764 \xA7f${hearts} \xA77HP\xA7r
\xA78\u2726 owner : \xA7f${this._owner.name}`;
  }
  //======================================================
   ...
   */
  getNearestDropedItemBehavior(options = { maxDistance: 2 }) {
    const nearbyDroppedItems = this._compagnon.dimension.getEntities({
      location: this._compagnon.location,
      ...options
    }).filter((e) => e.hasComponent(EntityComponentTypes3.Item)).sort((a, b) => this.nearestFromCompagnon(a.location, b.location));
    if (nearbyDroppedItems.length == 0) return false;
    debugLog(
      "[GetNearestDropedItemBehavior] - Compagnon Found " + nearbyDroppedItems.length + " item(s) in range " + options.maxDistance
    );
   ...
   */
  shouldAttackOwnerTargetBehavior({
    shouldIgnoreRange
  ...
  shouldAttackNearestMonsterMobs(options = { maxDistance: 10 }) {
    const hostileMobs = this._compagnon.dimension.getEntities({
      location: this._compagnon.location,
  ...
  shouldEatBehavior({ shouldEatAt, ShouldEatUntil } = {
    shouldEatAt: 6,
    ShouldEatUntil: 20
  }) {
    const hungerComponent = this._compagnon.getComponent(EntityComponentTypes3.Hunger);
  ...
  shouldAvoidMobsBehavior(props = { maxDistance: 3 }) {
    ...
    if (nearestMonster[0].typeId === "minecraft:creeper") {
      if (this.shouldProtectFromCreeperExplosionIfHasShield({
  ...
  shouldHealBehavior(props = {
    acceptableHealth: 20,
    critiqueProtectionAt: 6
  }) {
    const healthComponent = this._compagnon.getComponent(EntityComponentTypes3.Health);
  ...
  shouldProtectFromCreeperExplosionIfHasShield(options = {
    maxDistance: 5
  }) {
    let creeper = null;
    if (options.creeper) {
    ...
    const hasShield = this.getEquipableComponent().getEquipmentSlot(EquipmentSlot2.Offhand);
    try {
  ...
  shouldFarmCropBehavior() {
    ...
    const farmableLandsInArea = world4.getDimension(area.dimension).getBlocks(new BlockVolume3(area.waypoints.p1, area.waypoints.p2), {
  ...
  //=======================================================================
  farmingMobsBehavior() {
    const availableStackItem = this._compagnon.dimension.getEntities({
      location: this._compagnon.location,
      maxDistance: 5
    }).filter((e) => e.hasComponent(EntityComponentTypes3.Item)).sort((a, b) => this.nearestFromCompagnon(a.location, b.location));
    const mobs = world4.getDimension(this._compagnon.dimension.id).getEntities({
      location: this._compagnon.location,
      maxDistance: 20,
      families: ["mob"],
      excludeFamilies: ["player"]
    });
    const monstersMobs = mobs.filter((m) => m.getComponent(EntityComponentTypes3.TypeFamily)?.hasTypeFamily("monster")).sort((a, b) => this.nearestFromCompagnon(a.location, b.location));
    const farmableMobs = mobs.filter((m) => FOOD_MOBS.has(m.typeId)).sort((a, b) => this.nearestFromCompagnon(a.location, b.location));
    if (availableStackItem.length > 0) {
    ...
    } else if (monstersMobs.length > 0 && Vector3Utils.distance(this._compagnon.location, monstersMobs[0].location) < 5) {
      ...
      if (this.farming_behavior_datas.checkpoint !== null) {
        ...
        const topBlock = world4.getDimension(this._compagnon.dimension.id).getTopmostBlock({
          x: randomPoint.x,
          z: randomPoint.z
        });
        if (topBlock) {
...
// scripts/functions/createCompagnon.ts
import { spawnSimulatedPlayer } from "@minecraft/server-gametest";
import { GameMode } from "@minecraft/server";
function createCompagnon(player) {
...
// scripts/events/player.ts
world5.afterEvents.playerSpawn.subscribe(({ player }) => {
  const compagnonData = CompagnonDBManager.getCompagnon(player);
  if (compagnonData) {
    const compagnonManager = createCompagnon(player);
    compagnonManager.compagnon.getComponent(EntityComponentTypes4.Health)?.setCurrentValue(compagnonData.health);
    compagnonManager.updateBehavior(compagnonData.forced_behavior);
  }
});
world5.afterEvents.playerLeave.subscribe(({ playerId }) => {
...
world5.afterEvents.playerInventoryItemChange.subscribe((event) => {
  const { player } = event;
  ...
  if (event.block.typeId === MinecraftBlockTypes.CarvedPumpkin) {
    ...
    if (isSpawningCompagnon) {
      if (CompagnonDBManager.hasCompagnon(event.player)) {
        ...
        blocks.forEach(
          (b) => event.player.dimension.setBlockType(
            b.location,
...
// scripts/events/system.ts
import { system } from "@minecraft/server";

// scripts/customComponents/CompagnonStickCustomComponent.ts
var CompagnonStickCustomComponent = class {
  constructor() {
  ...
  onUse(event, param) {
    ...
    let usedOn;
    const entityDistance = useOnEntity.length > 0 ? useOnEntity[0].distance : void 0;
    const blockDistance = useOnBlock ? Vector3Utils.distance(useOnBlock.block.location, source.location) : void 0;
...
if (isDebug) {
  ...
  system2.runTimeout(() => {
    ...
    for (const {
      ...
      const compagnonManager = createCompagnon(player);
      compagnonManager.compagnon.getComponent(EntityComponentTypes5.Health)?.setCurrentValue(compagnon.health);
